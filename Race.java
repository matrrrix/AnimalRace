	...
	private void prepareToRace() {
		...
		while (input != 's' && input != 'S') {
			...
			switch (input) {
				case 'T':
				case 't':
					Racer playerTortoise = new Tortoise("Tortoise", START_LINE, yPos);
					racerList.add(playerTortoise);
					yPos += RACER_SPACE;
					break;
				case 'H':
				case 'h':
					Racer playerHare = new Hare("Hare", START_LINE, yPos);
					racerList.add(playerHare);
					yPos += RACER_SPACE;
					break;
				case 'P':
				case 'p':
					Racer playerPython = new Python("Python", START_LINE, yPos);
					racerList.add(playerPython);
					yPos += RACER_SPACE;
					break;
				case 'K':
				case 'k':
					Racer playerKangaroo = new Kangaroo("Kangaroo", START_LINE,yPos);
					racerList.add(playerKangaroo);
					yPos += RACER_SPACE;
					break;
				default:
					JOptionPane.showMessageDialog(this, "Racer does not exist");	
			}
	...
	private void reportRaceResults() {
		...
		for (int i = 0; i < racerList.size(); i++) {
			if (racerList.get(i).getX() > finishX) {
				racerList.get(i).morph(racePanel.getGraphics());
